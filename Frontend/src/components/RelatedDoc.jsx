...
const RelatedDoc = ({speciality, docId}) => {
  ...
  return (
   <div className='flex flex-col items-center gap-4 my-16 md:mx-10 text-black'>
      <h1 className='text-3xl font-medium'>Top Doctors</h1>
      <p className='sm:w-1/3 text-center text-sm '>Find the right doctor for your needs</p>
      <div className='w-full grid grid-cols-3 md:grid-cols-5 gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
            ...
            className='border border-green-600 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
               ...
               <div className='p-4'>
                  <div className={`flex items-center gap-2 text-sm text-center ${item.available ? 'text-green-600' : 'text-red-600'}`}>
                     <p className={`w-2 h-2 ${item.available ? 'bg-green-600' : 'bg-red-600'}  rounded-full`}></p><p>{item.available ? 'Available' : 'Not Available'}</p>
                  </div>
      ...
      className='bg-green-300 text-gray-700 px-12 py-4 rounded-full mt-10'>more</button>
   </div>
  )
}
